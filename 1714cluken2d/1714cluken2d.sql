-- Callum Luken
-- Exercise 2D Summary Multi-table Join Queries
...
GROUP BY Person.PersonId, Person.LastName, Person.FirstName, Building.City, Building.BuildingId, Building.BuildingName, Apartment.ApartmentNum
...
SELECT        Invoice.InvoiceId, Invoice.InvoiceDate, Person.LastName, Building.BuildingId, Apartment.ApartmentNum, SUM(LineItem.Amount) AS [Total Billed], Receipt.Amount AS Received
FROM            Invoice INNER JOIN
                         Apartment ON Invoice.ApartmentId = Apartment.ApartmentId INNER JOIN
                         Person ON Apartment.TenantId = Person.PersonId INNER JOIN
                         Building ON Apartment.BuildingId = Building.BuildingId INNER JOIN
...
-- 2D.7
SELECT        Person.PersonId, Person.LastName, Invoice.InvoiceId, Invoice.InvoiceDate, SUM(LineItem.Amount) AS Billed, Receipt.Amount AS Received
FROM            Person INNER JOIN
